...
static zend_always_inline zend_result add_function_fast(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (EXPECTED(type_pair == TYPE_PAIR(IS_LONG, IS_LONG))) {
		...
		return SUCCESS;
	} else if (type_pair == TYPE_PAIR(IS_BIGINT, IS_DOUBLE)) {
		double d = zend_int_to_double(op1) + Z_DVAL_P(op2);
		if (result == op1) {
			zval_ptr_dtor_nogc(result);
		}
		ZVAL_DOUBLE(result, d);
		return SUCCESS;
	} else if (type_pair == TYPE_PAIR(IS_DOUBLE, IS_BIGINT)) {
		ZVAL_DOUBLE(result, Z_DVAL_P(op1) + zend_int_to_double(op2));
		return SUCCESS;
	} else if (Z_IS_INT_P(op1) && Z_IS_INT_P(op2)) {
		zval tmp;
		zend_int_add(&tmp, op1, op2);
		if (result == op1) {
			zval_ptr_dtor_nogc(result);
		}
...
static zend_result ZEND_FASTCALL pow_function_base(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (EXPECTED(type_pair == TYPE_PAIR(IS_LONG, IS_LONG))) {
		if (Z_LVAL_P(op2) >= 0) {
			...
			while (i >= 1) {
				...
				if (i % 2) {
					...
					if (overflow) {
						return zend_int_pow(result, op1, op2);
					}
				} else {
					i /= 2;
		...
		double d = safe_pow(zend_int_to_double(op1), Z_DVAL_P(op2));
		if (result == op1) {
			zval_ptr_dtor_nogc(result);
		}
		ZVAL_DOUBLE(result, d);
		...
		ZVAL_DOUBLE(result, safe_pow(Z_DVAL_P(op1), zend_int_to_double(op2)));
		return SUCCESS;
	} else if (Z_IS_INT_P(op1) && Z_IS_INT_P(op2)) {
		if (zend_int_sign(op2) < 0) {
			/* A negative exponent keeps the float path. */
			double d = safe_pow(zend_int_to_double(op1), zend_int_to_double(op2));
			if (result == op1) {
				zval_ptr_dtor_nogc(result);
			}
			ZVAL_DOUBLE(result, d);
		...
		zval tmp;
		if (zend_int_pow(&tmp, op1, op2) == FAILURE) {
			if (result != op1) {
				ZVAL_UNDEF(result);
			}
			return FAILURE;
...
/* Divides two logical integers, at least one boxed, storing an exact quotient
 * as a canonical integer and an inexact one as a double. The divisor is
 * nonzero. result must not alias either operand. */
static void div_int_exact_or_double(zval *result, const zval *op1, const zval *op2)
{
	...
	if (zend_bigint_sign(rem) == 0) {
		zend_bigint_free(rem);
		zend_int_from_bigint(result, quot);
	} else {
		zend_bigint_free(rem);
		zend_bigint_free(quot);
		ZVAL_DOUBLE(result, zend_int_to_double(op1) / zend_int_to_double(op2));
	}
}
...
static zend_div_status ZEND_FASTCALL div_function_base(zval *result, const zval *op1, const zval *op2) /* {{{ */
{
	...
	if (EXPECTED(type_pair == TYPE_PAIR(IS_LONG, IS_LONG))) {
		if (Z_LVAL_P(op2) == 0) {
			...
			/* The quotient ZEND_LONG_MAX + 1 is exact but does not fit
			 * zend_long, so it boxes. */
		...
		zval tmp;
		div_int_exact_or_double(&tmp, op1, op2);
		if (result == op1) {
			zval_ptr_dtor_nogc(result);
		}
		ZVAL_COPY_VALUE(result, &tmp);
 ...
 * any other value is copied verbatim, so the operator applies its existing
 * zendi_try_get_long coercion and error. */
...
/* Reduce a holder that isn't already a logical int (long or bigint) to a
 * plain long in place, via the existing zendi_try_get_long coercion. Object
 * overloads are handled by the caller before any holder exists, so this never
 * returns early. */
static zend_always_inline zend_result zend_holder_coerce_to_long(zval *holder)
{
	if (Z_IS_INT_P(holder)) {
		return SUCCESS;
	}
	bool failed;
	zend_long lval = zendi_try_get_long(holder, &failed);
	if (UNEXPECTED(failed)) {
		return FAILURE;
	}
	zval_ptr_dtor(holder);
	ZVAL_LONG(holder, lval);
	return SUCCESS;
}
...
ZEND_API zend_result ZEND_FASTCALL mod_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (Z_TYPE_P(op1) == IS_BIGINT || Z_TYPE_P(op2) == IS_BIGINT) {
		...
		if (Z_TYPE_P(b) == IS_LONG && Z_LVAL_P(b) == 0) {
			/* modulus by zero */
	...
	if (Z_TYPE_P(op1) == IS_STRING || Z_TYPE_P(op2) == IS_STRING) {
		...
		zval op1_holder, op2_holder;
		bool op1_is_bigint, op2_is_bigint;
		if (UNEXPECTED(zend_op_to_bigint_holder(op1, &op1_holder, &op1_is_bigint) == FAILURE)) {
			if (op1 != result) {
				ZVAL_UNDEF(result);
			}
			return FAILURE;
		}
		if (UNEXPECTED(zend_op_to_bigint_holder(op2, &op2_holder, &op2_is_bigint) == FAILURE)) {
		...
		if (UNEXPECTED(zend_holder_coerce_to_long(&op1_holder) == FAILURE)
				|| UNEXPECTED(zend_holder_coerce_to_long(&op2_holder) == FAILURE)) {
			zval_ptr_dtor(&op1_holder);
		...
		if (op1_is_bigint || op2_is_bigint) {
			if (Z_TYPE(op2_holder) == IS_LONG && Z_LVAL(op2_holder) == 0) {
				zval_ptr_dtor(&op1_holder);
				zval_ptr_dtor(&op2_holder);
				/* modulus by zero */
				if (EG(current_execute_data) && !CG(in_compilation)) {
			...
			zval tmp;
			zend_int_mod(&tmp, &op1_holder, &op2_holder);
			zval_ptr_dtor(&op1_holder);
			zval_ptr_dtor(&op2_holder);
			if (op1 == result) {
				zval_ptr_dtor(result);
		...
		op1_lval = Z_LVAL(op1_holder);
		op2_lval = Z_LVAL(op2_holder);
		zval_ptr_dtor(&op1_holder);
		zval_ptr_dtor(&op2_holder);
	} else {
...
/* Shared IS_STRING operand arm for the bitwise operators. */
#define bitwise_string_bigint_op(op1, op1_lval, op2, op2_lval, result, opcode, sigil, int_fn) \
	do { \
		...
		if (op1_is_bigint || op2_is_bigint) { \
			zval tmp; \
			int_fn(&tmp, &op1_holder, &op2_holder); \
			zval_ptr_dtor(&op1_holder); \
			zval_ptr_dtor(&op2_holder); \
			if (op1 == result) { \
				zval_ptr_dtor(result); \
...
ZEND_API zend_result ZEND_FASTCALL bitwise_or_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (Z_TYPE_P(op1) == IS_STRING || Z_TYPE_P(op2) == IS_STRING) {
		bitwise_string_bigint_op(op1, op1_lval, op2, op2_lval, result, ZEND_BW_OR, "|", zend_int_or);
	} else {
		if (UNEXPECTED(Z_TYPE_P(op1) != IS_LONG)) {
			bool failed;
...
ZEND_API zend_result ZEND_FASTCALL bitwise_and_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (Z_TYPE_P(op1) == IS_STRING || Z_TYPE_P(op2) == IS_STRING) {
		bitwise_string_bigint_op(op1, op1_lval, op2, op2_lval, result, ZEND_BW_AND, "&", zend_int_and);
	} else {
		if (UNEXPECTED(Z_TYPE_P(op1) != IS_LONG)) {
			bool failed;
...
ZEND_API zend_result ZEND_FASTCALL bitwise_xor_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (Z_TYPE_P(op1) == IS_STRING || Z_TYPE_P(op2) == IS_STRING) {
		bitwise_string_bigint_op(op1, op1_lval, op2, op2_lval, result, ZEND_BW_XOR, "^", zend_int_xor);
	} else {
		if (UNEXPECTED(Z_TYPE_P(op1) != IS_LONG)) {
			bool failed;
...
ZEND_API zend_result ZEND_FASTCALL shift_left_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if ((Z_IS_INT_P(op1) && Z_IS_INT_P(op2))
		...
		zval tmp;
		if (zend_int_shift_left(&tmp, a, b) == FAILURE) {
			/* The backend's over-reach ArithmeticError has been thrown. */
			if (op1 != result) {
				ZVAL_UNDEF(result);
			}
			return FAILURE;
	...
	if (Z_TYPE_P(op1) == IS_STRING || Z_TYPE_P(op2) == IS_STRING) {
		...
		if (op1_is_bigint || op2_is_bigint) {
			...
			zval tmp;
			if (zend_int_shift_left(&tmp, &op1_holder, &op2_holder) == FAILURE) {
				/* The backend's over-reach ArithmeticError has been thrown. */
				zval_ptr_dtor(&op1_holder);
				zval_ptr_dtor(&op2_holder);
				if (op1 != result) {
	...
	if (UNEXPECTED((zend_ulong)op2_lval >= SIZEOF_ZEND_LONG * 8)) {
		if (EXPECTED(op2_lval > 0)) {
			zval a, b, tmp;
			ZVAL_LONG(&a, op1_lval);
			ZVAL_LONG(&b, op2_lval);
			if (zend_int_shift_left(&tmp, &a, &b) == FAILURE) {
				if (op1 != result) {
					ZVAL_UNDEF(result);
				}
...
ZEND_API zend_result ZEND_FASTCALL shift_right_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (Z_TYPE_P(op1) == IS_STRING || Z_TYPE_P(op2) == IS_STRING) {
		...
		if (op1_is_bigint || op2_is_bigint) {
			...
			zval tmp;
			zend_int_shift_right(&tmp, &op1_holder, &op2_holder);
			zval_ptr_dtor(&op1_holder);
			zval_ptr_dtor(&op2_holder);
			if (op1 == result) {
				zval_ptr_dtor(result);
