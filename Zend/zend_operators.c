...
static zend_always_inline void zend_bigint_release_result_alias(zval *result, zval *op1, zval *op2)
{
	if ((result == op1 || result == op2) && Z_TYPE_P(result) == IS_BIGINT) {
...
ZEND_API void ZEND_FASTCALL zend_bigint_long_overflow_add(zval *result, zend_long a, zend_long b)
{
	...
	zend_bigint_long_add_long(big, a, b);
...
{
	zend_bigint *big = zend_bigint_init();
	zend_bigint_long_mul_long(big, a, b);
	ZVAL_BIGINT(result, big);
}
...
static zend_always_inline zend_result mul_function_fast(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (EXPECTED(type_pair == TYPE_PAIR(IS_LONG, IS_LONG))) {
		fast_long_mul_function(result, op1, op2);
		return SUCCESS;
	} else if (EXPECTED(type_pair == TYPE_PAIR(IS_DOUBLE, IS_DOUBLE))) {
		ZVAL_DOUBLE(result, Z_DVAL_P(op1) * Z_DVAL_P(op2));
		return SUCCESS;
		...
		zend_bigint *r = zend_bigint_init();
		zend_bigint_mul(r, Z_BIG_P(op1), Z_BIG_P(op2));
		zend_bigint_release_result_alias(result, op1, op2);
		zend_bigint_result(result, r);
		return SUCCESS;
	} else if (type_pair == TYPE_PAIR(IS_BIGINT, IS_LONG)) {
		zend_bigint *r = zend_bigint_init();
		zend_bigint_mul_long(r, Z_BIG_P(op1), Z_LVAL_P(op2));
		zend_bigint_release_result_alias(result, op1, op2);
		zend_bigint_result(result, r);
		return SUCCESS;
	} else if (type_pair == TYPE_PAIR(IS_LONG, IS_BIGINT)) {
		zend_bigint *r = zend_bigint_init();
		zend_bigint_mul_long(r, Z_BIG_P(op2), Z_LVAL_P(op1));
		zend_bigint_release_result_alias(result, op1, op2);
		zend_bigint_result(result, r);
		return SUCCESS;
	} else {
...
static zend_never_inline zend_result ZEND_FASTCALL bitwise_or_function_bigint(zval *result, zval *op1, zval *op2)
{
	...
	zend_bigint *r = zend_bigint_init();
	if (op1_is_big && op2_is_big) {
		zend_bigint_or(r, Z_BIG_P(op1), Z_BIG_P(op2));
	} else if (op1_is_big) {
		zend_bigint_or_long(r, Z_BIG_P(op1), op2_lval);
	} else {
		zend_bigint_or_long(r, Z_BIG_P(op2), op1_lval);
	}
	zend_bigint_release_result_alias(result, op1, op2);
	zend_bigint_result(result, r);
	return SUCCESS;
...
static zend_never_inline zend_result ZEND_FASTCALL bitwise_and_function_bigint(zval *result, zval *op1, zval *op2)
{
	...
	zend_bigint *r = zend_bigint_init();
	if (op1_is_big && op2_is_big) {
		zend_bigint_and(r, Z_BIG_P(op1), Z_BIG_P(op2));
	} else if (op1_is_big) {
		zend_bigint_and_long(r, Z_BIG_P(op1), op2_lval);
	} else {
		zend_bigint_and_long(r, Z_BIG_P(op2), op1_lval);
	}
	zend_bigint_release_result_alias(result, op1, op2);
	zend_bigint_result(result, r);
	return SUCCESS;
...
static zend_never_inline zend_result ZEND_FASTCALL bitwise_xor_function_bigint(zval *result, zval *op1, zval *op2)
{
	...
	zend_bigint *r = zend_bigint_init();
	if (op1_is_big && op2_is_big) {
		zend_bigint_xor(r, Z_BIG_P(op1), Z_BIG_P(op2));
	} else if (op1_is_big) {
		zend_bigint_xor_long(r, Z_BIG_P(op1), op2_lval);
	} else {
		zend_bigint_xor_long(r, Z_BIG_P(op2), op1_lval);
	}
	zend_bigint_release_result_alias(result, op1, op2);
	zend_bigint_result(result, r);
	return SUCCESS;
